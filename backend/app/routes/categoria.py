...
def get_by_id(id_categoria: UUID, session: Session = Depends(get_session)):
    return service.get_by_id(session, id_categoria)

@router.post("/", response_model=CategoriaRead, status_code=201)
def create(data: CategoriaCreate, session: Session = Depends(get_session)):
    return service.create(session, data)

@router.patch("/{id_categoria}", response_model=CategoriaRead)
def update(id_categoria: UUID, data: CategoriaUpdate, session: Session = Depends(get_session)):
    return service.update(session, id_categoria, data)

@router.delete("/{id_categoria}")
def delete(id_categoria: UUID, session: Session = Depends(get_session)):
