...
def get_malla(id_carrera: UUID, session: Session = Depends(get_session)):
    return service.get_malla_by_carrera(session, id_carrera)

@router.post("/", response_model=MallaRead, status_code=201)
def add_materia(data: MallaCreate, session: Session = Depends(get_session)):
    return service.add_materia(session, data)

@router.delete("/{id_carrera}/{id_materia}")
def remove_materia(id_carrera: UUID, id_materia: UUID, session: Session = Depends(get_session)):
