...
def get_by_id(id_carrera: UUID, session: Annotated[Session, Depends(get_session)]):
    return service.get_by_id(session, id_carrera)

@router.post("/", response_model=CarreraRead, status_code=201)
def create(data: CarreraCreate, session: Annotated[Session, Depends(get_session)]):
    return service.create(session, data)

@router.patch("/{id_carrera}", response_model=CarreraRead)
def update(id_carrera: UUID, data: CarreraUpdate, session: Annotated[Session, Depends(get_session)]):
    return service.update(session, id_carrera, data)

@router.delete("/{id_carrera}")
def delete(id_carrera: UUID, session: Annotated[Session, Depends(get_session)]):
