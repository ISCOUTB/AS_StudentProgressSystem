...
def get_by_id(id_estudiante: UUID, session: Annotated[Session, Depends(get_session)]):
    return service.get_by_id(session, id_estudiante)

@router.post("/", response_model=EstudianteRead, status_code=201)
def create(data: EstudianteCreate, session: Annotated[Session, Depends(get_session)]):
    return service.create(session, data)

@router.patch("/{id_estudiante}", response_model=EstudianteRead)
def update(id_estudiante: UUID, data: EstudianteUpdate, session: Annotated[Session, Depends(get_session)]):
    return service.update(session, id_estudiante, data)

@router.delete("/{id_estudiante}")
def delete(id_estudiante: UUID, session: Annotated[Session, Depends(get_session)]):
