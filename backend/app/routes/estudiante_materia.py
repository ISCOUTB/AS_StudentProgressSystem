...
def create(data: EstudianteMateriaCreate, session: Session = Depends(get_session)):
    return service.create(session, data)

@router.patch("/{id_estudiante}/{id_materia}", response_model=EstudianteMateriaRead)
def update(id_estudiante: UUID, id_materia: UUID, data: EstudianteMateriaUpdate, session: Session = Depends(get_session)):
    return service.update(session, id_estudiante, id_materia, data)

@router.delete("/{id_estudiante}/{id_materia}")
def delete(id_estudiante: UUID, id_materia: UUID, session: Session = Depends(get_session)):
