...
def get_by_id(id_logro: UUID, session: Session = Depends(get_session)):
    return service.get_by_id(session, id_logro)

@router.post("/", response_model=LogroRead, status_code=201)
def create(data: LogroCreate, session: Session = Depends(get_session)):
    return service.create(session, data)

@router.patch("/{id_logro}", response_model=LogroRead)
def update(id_logro: UUID, data: LogroUpdate, session: Session = Depends(get_session)):
    return service.update(session, id_logro, data)

@router.delete("/{id_logro}")
def delete(id_logro: UUID, session: Session = Depends(get_session)):
