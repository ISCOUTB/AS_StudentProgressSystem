...
def get_by_logro(id_logro: UUID, session: Session = Depends(get_session)):
    return service.get_by_logro(session, id_logro)

@router.post("/", response_model=LogroMateriaRead, status_code=201)
def create(data: LogroMateriaCreate, session: Session = Depends(get_session)):
    return service.create(session, data)

@router.delete("/{id_logromateria}")
def delete(id_logromateria: UUID, session: Session = Depends(get_session)):
