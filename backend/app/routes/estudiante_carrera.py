...
def create(data: EstudianteCarreraCreate, session: Session = Depends(get_session)):
    return service.create(session, data)

@router.patch("/{id_estudiante}/{id_carrera}", response_model=EstudianteCarreraRead)
def update(id_estudiante: UUID, id_carrera: UUID, data: EstudianteCarreraUpdate, session: Session = Depends(get_session)):
    return service.update(session, id_estudiante, id_carrera, data)

@router.delete("/{id_estudiante}/{id_carrera}")
def delete(id_estudiante: UUID, id_carrera: UUID, session: Session = Depends(get_session)):
