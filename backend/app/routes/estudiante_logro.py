...
def create(data: EstudianteLogroCreate, session: Annotated[Session, Depends(get_session)]):
    return service.create(session, data)

@router.patch("/{id_estudiante}/{id_logromateria}", response_model=EstudianteLogroRead)
def update(id_estudiante: UUID, id_logromateria: UUID, data: EstudianteLogroUpdate, session: Annotated[Session, Depends(get_session)]):
    return service.update(session, id_estudiante, id_logromateria, data)

@router.delete("/{id_estudiante}/{id_logromateria}")
def delete(id_estudiante: UUID, id_logromateria: UUID, session: Annotated[Session, Depends(get_session)]):
